...
# Test for pro categories controller
class ProdCategoriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create prod_category' do
    assert_difference('ProdCategory.count') do
      post prod_categories_url,
           params: { prod_category: { categoryId: @prod_category.categoryId, name: @prod_category.name } }
    end
  ...
  test 'should update prod_category' do
    patch prod_category_url(@prod_category),
          params: { prod_category: { categoryId: @prod_category.categoryId, name: @prod_category.name } }
